...
class ReportingDAO:
    ...
    async def get_subscriber_count(self) -> int:
        ...
        return self.session.exec(select(func.count()).select_from(Subscriber)).one()
    ...
    async def get_recent_logs(self, days: int = 7, limit: int = 10) -> List[Log]:
        ...
        query = (
            select(Log)
            .where(Log.timestamp >= cutoff_date)
    ...
    async def get_status_distribution(self) -> List[Dict[str, Any]]:
        ...
        query = select(Log.status_code, func.count(Log.id).label("count")).group_by(
            Log.status_code
        )
