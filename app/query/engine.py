...
class QueryEngine:
    ...
    def build_consolidated_query(
        ...
        if not user_defined_calcs and not system_sql_calcs:
            return self._build_system_fields_only_query(
                base_cte, system_field_calcs, aggregation_level
            )
        ...
        if system_sql_calcs:
            print(
                f"Debug: Found {len(system_sql_calcs)} system SQL calculations, executing them separately"
            )
        ...
        query = self._build_final_combined_query(
            ...
            user_defined_calcs,
            [],
            aggregation_level,  # Empty system_sql_calcs for now
        )
    ...
    def _build_base_cte(
        ...
        base_columns = [Deal.dl_nbr.label("deal_number"), TrancheBal.cycle_cde.label("cycle_code")]
        ...
        for calc in calculations:
            if calc.is_user_defined():
                ...
                source_model_value = (
                    calc.source_model.value
                    if hasattr(calc.source_model, "value")
                    else calc.source_model
                )
        ...
        model_map = {"Deal": Deal, "Tranche": Tranche, "TrancheBal": TrancheBal}
        ...
        if TrancheBal in required_models:
            base_query = base_query.join(
                TrancheBal,
                and_(Tranche.dl_nbr == TrancheBal.dl_nbr, Tranche.tr_id == TrancheBal.tr_id),
            )
    ...
    def _build_user_defined_calculation_cte(
        self, base_cte, calc: "Calculation", aggregation_level: str
    ):
        """Build a CTE for a single user-defined aggregated calculation."""

        # Get the source model value (handle enum)
        source_model_value = (
            calc.source_model.value if hasattr(calc.source_model, "value") else calc.source_model
        )
        ...
        if not hasattr(base_cte.c, field_name):
            ...
            raise ValueError(
                f"Field '{field_name}' not found in base CTE. Available fields: {available_fields}"
            )
        ...
        agg_func_value = (
            calc.aggregation_function.value
            if hasattr(calc.aggregation_function, "value")
            else calc.aggregation_function
        )
        ...
        if agg_func_value == "SUM":
            ...
            if not calc.weight_field:
                raise ValueError(
                    f"Weighted average calculation '{calc.name}' requires weight_field"
                )
            weight_field_name = f"{source_model_value}_{calc.weight_field}"
            if not hasattr(base_cte.c, weight_field_name):
                available_fields = [col.name for col in base_cte.c]
                raise ValueError(
                    f"Weight field '{weight_field_name}' not found in base CTE. Available fields: {available_fields}"
                )
        ...
        calc_query = (
            self.dw_db.query(*select_columns).select_from(base_cte).group_by(*group_columns)
        )
        ...
        class MockCTE:
            def __init__(self, sql, result_column, aggregation_level):
                ...
                self.c = type(
                    "MockColumns",
                    (),
                    {"dl_nbr": text("dl_nbr"), result_column: text(result_column)},
                )()
                if aggregation_level == "tranche":
    ...
    def _inject_filters_into_system_sql(
        ...
        if not deal_tranche_map:
            ...
            for dl_nbr, selected_tranches in deals_with_specific_tranches:
                ...
                escaped_tranches = ",".join(
                    f"'{t.replace(chr(39), chr(39)+chr(39))}'" for t in selected_tranches
                )
                deal_condition_parts.append(
                    f"(deal.dl_nbr = {dl_nbr} AND tranche.tr_id IN ({escaped_tranches}))"
                )
    ...
    def _build_system_fields_only_query(
        self, base_cte, system_field_calcs: List["Calculation"], aggregation_level: str
    ):
        """Build query for system fields only (no aggregation needed)."""

        # Start with base columns
        select_columns = [base_cte.c.deal_number, base_cte.c.cycle_code]
        ...
        for calc in system_field_calcs:
            ...
            if not hasattr(base_cte.c, field_name):
                available_fields = [col.name for col in base_cte.c]
                raise ValueError(
                    f"System field '{field_name}' not found in base CTE. Available fields: {available_fields}"
                )
    ...
    def _build_final_combined_query(
        ...
        for calc in user_defined_calcs:
            ...
            final_query = final_query.add_columns(getattr(calc_cte.c, column_name).label(calc.name))
            ...
            if aggregation_level == "tranche":
                ...
                final_query = final_query.outerjoin(
                    calc_cte, base_cte.c.deal_number == calc_cte.c.calc_deal_nbr
                )
        ...
        for calc in system_sql_calcs:
            ...
            if hasattr(sql_cte, "sql"):  # This is our MockCTE
                ...
                final_query = final_query.add_columns(literal_column("NULL").label(calc.name))
            else:
                ...
                if aggregation_level == "tranche":
                    ...
                    final_query = final_query.outerjoin(
                        sql_cte, base_cte.c.deal_number == sql_cte.c.dl_nbr
                    )
    ...
    def preview_calculation_sql(
        ...
        return {
            ...
            "sample_parameters": {"deal_tranche_mapping": deal_tranche_map, "cycle": sample_cycle},
        }
        ...
        for calc in system_sql_calcs:
            if calc.name in system_sql_ctes:
                ...
                if aggregation_level == "tranche":
                    ...
                    join_clause = (
                        f"LEFT OUTER JOIN {cte_name} ON base_data.deal_number = {cte_name}.dl_nbr"
                    )
                join_clauses.append(join_clause)
...
{chr(10).join(join_clauses)}"""
        ...
        if hasattr(query, "statement"):
            ...
            return str(
                query.statement.compile(
                    dialect=self.dw_db.bind.dialect, compile_kwargs={"literal_binds": True}
                )
            )
    ...
    def get_calculations_by_names(self, names: List[str]) -> List["Calculation"]:
        ...
        return (
            self.config_db.query(Calculation)
    ...
    def process_report_results(
        self, results: List[Any], calculations: List["Calculation"], aggregation_level: str
    ) -> List[Dict[str, Any]]:
        ...
        for result in results:
            ...
            for calc in calculations:
                ...
                if calc_value is None and hasattr(result, "_fields"):
                    ...
                    if (
                        normalized_name
                        and not normalized_name[0].isalpha()
                        and normalized_name[0] != "_"
                    ):
