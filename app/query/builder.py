...
class QueryBuilder:
    ...
    def build_query(self, params: QueryParameters) -> Select:
        ...
        if not user_calculations:
            return self._build_system_fields_only_query(
                base_cte, system_fields, params.aggregation_level
            )
    ...
    def build_preview(self, params: QueryParameters) -> PreviewResult:
        ...
        query_result = QueryResult(
            sql=sql_string, parameters=self._extract_query_parameters(params), columns=columns
        )

        return PreviewResult(query_result=query_result, calculation_summary=calculation_summary)

    def _build_base_data_cte(
        self, params: QueryParameters, all_calculations: List[CalculationDefinition]
    ) -> Any:
        """
    ...
    def _build_calculation_cte(
        self, base_cte: Any, calculation: CalculationDefinition, aggregation_level: AggregationLevel
    ) -> Any:
        """Build a CTE for a single user-defined calculation."""
        if not calculation.is_user_defined():
            raise ValueError(
                f"Cannot build calculation CTE for non-user-defined calculation: {calculation.name}"
            )
        ...
        if not hasattr(base_cte.c, field_name):
            ...
            raise ValueError(
                f"Field '{field_name}' not found in base CTE. Available: {available_fields}"
            )
        ...
        calc_query = select(*select_columns).select_from(base_cte).group_by(*group_columns)
    ...
    def _build_final_query(
        ...
        for calc in user_calculations:
            ...
            if aggregation_level == AggregationLevel.TRANCHE:
                ...
                final_query = final_query.outerjoin(
                    calc_cte, base_cte.c.deal_number == calc_cte.c.calc_deal_number
                )
    ...
    def _build_aggregation_expression(
        ...
        source_field,
        weight_field: Optional[FieldDefinition],
        base_cte: Any,
    ):
    ...
    def _build_deal_tranche_conditions(self, filters: List[DealTrancheFilter]):
        ...
        for deal_filter in filters:
            if deal_filter.includes_all_tranches():
                ...
                specific_tranche_conditions.append(
                    and_(
    ...
    def _get_required_fields(
        self, calculations: List[CalculationDefinition]
    ) -> Set[FieldDefinition]:
    ...
    def _compile_query_to_sql(self, query: Select) -> str:
        ...
        return str(
            query.compile(
                dialect=self.dw_session.bind.dialect, compile_kwargs={"literal_binds": True}
            )
        )
    ...
    def _initialize_table_registry(self) -> Dict[str, Any]:
        ...
        return {"Deal": Deal, "Tranche": Tranche, "TrancheBal": TrancheBal}

    def _initialize_relationships(self) -> List[TableRelationship]:
        """Initialize the registry of table relationships."""
        return [
            TableRelationship(
                parent_table="Deal", child_table="Tranche", join_conditions=[("dl_nbr", "dl_nbr")]
            ),
            TableRelationship(
