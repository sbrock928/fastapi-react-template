...
class CalculationDAO:
    ...
    def get_all_calculations(
        ...
        return query.order_by(
            Calculation.calculation_type,  # System calculations first
            Calculation.name
        ).all()
    
    def get_user_defined_calculations(self, group_level: Optional[GroupLevel] = None) -> List[Calculation]:
        """Get only user-defined calculations."""
        return self.get_all_calculations(group_level, CalculationType.USER_DEFINED)
    
    def get_system_field_calculations(self, group_level: Optional[GroupLevel] = None) -> List[Calculation]:
        """Get only system field calculations."""
        return self.get_all_calculations(group_level, CalculationType.SYSTEM_FIELD)
    
    def get_system_sql_calculations(self, group_level: Optional[GroupLevel] = None) -> List[Calculation]:
        """Get only system SQL calculations."""
        return self.get_all_calculations(group_level, CalculationType.SYSTEM_SQL)
    
    def get_system_calculations(self, group_level: Optional[GroupLevel] = None) -> List[Calculation]:
        ...
        query = self.db.query(Calculation).filter(
            ...
            Calculation.is_system_managed == True
        )
    ...
    def get_editable_calculations(self, group_level: Optional[GroupLevel] = None) -> List[Calculation]:
        """Get only user-editable calculations."""
        query = self.db.query(Calculation).filter(
            Calculation.is_active == True,
            Calculation.is_system_managed == False
        )
    ...
    def count_by_type(self) -> dict:
        ...
        for calc_type in CalculationType:
            count = self.db.query(Calculation).filter(
                Calculation.calculation_type == calc_type,
                Calculation.is_active == True
            ).count()
    ...
    def get_system_field_by_source_and_field(self, source_model: str, field_name: str, group_level: GroupLevel) -> Optional[Calculation]:
