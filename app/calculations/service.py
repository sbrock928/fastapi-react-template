...
class CalculationService:
    ...
    def get_available_calculations(
        self, group_level: Optional[str] = None, calculation_type: Optional[str] = None
    ...
    def get_user_defined_calculations(
        self, group_level: Optional[str] = None
    ...
    def create_user_defined_calculation(
        self, request: UserDefinedCalculationCreate, user_id: str = "api_user"
    ) -> CalculationResponse:
        ...
        if existing:
            raise CalculationAlreadyExistsError(
                f"Calculation with name '{request.name}' already exists at {request.group_level} level"
            )

        # Validate weighted average has weight field
        if (
            request.aggregation_function == AggregationFunction.WEIGHTED_AVG
            and not request.weight_field
        ):
    ...
    def update_user_defined_calculation(
        self, calc_id: int, request: UserDefinedCalculationCreate
    ) -> CalculationResponse:
        ...
        if calculation.calculation_type != CalculationType.USER_DEFINED:
            raise InvalidCalculationError(
                "Only user-defined calculations can be updated via this method"
            )
        ...
        if existing and existing.id != calc_id:
            raise CalculationAlreadyExistsError(
                f"Another calculation with name '{request.name}' already exists at {request.group_level} level"
            )
    ...
    def create_system_sql_calculation(
        self, request: SystemSQLCalculationCreate, user_id: str = "system"
    ) -> CalculationResponse:
        ...
        validation_result = self.sql_validator.validate_custom_sql(
            request.raw_sql, request.group_level.value, request.result_column_name
        )
    ...
    def validate_system_sql(
        self, sql_text: str, group_level: str, result_column_name: str
    ) -> Dict[str, Any]:
        """Validate system SQL without saving."""
        validation_result = self.sql_validator.validate_custom_sql(
            sql_text, group_level, result_column_name
        )
    ...
    def delete_calculation(self, calc_id: int) -> dict:
        ...
        if calculation.is_system_managed:
            raise InvalidCalculationError(
                f"Cannot delete system-managed calculation '{calculation.name}'. System calculations are read-only."
            )
